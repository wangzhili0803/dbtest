...
/**
 * Created by wli on 16/2/25. 用户信息缓存 流程： 1、如果内存中有则从内存中获取 2、如果内存中没有则从 db 中获取 3、如果 db 中没有则通过调用开发者设置的回调 LCChatProfileProvider.fetchProfiles 来获取
 * 同时获取到的数据会缓存到内存与 db
 */
public class LCIMProfileCache {
    ...
    private final Map<String, LCChatKitUser> userMap;
    ...
    /**
     * 因为只有在第一次的时候需要设置 Context 以及 clientId，所以单独拎出一个函数主动调用初始化 避免 getInstance 传入过多参数
     */
    ...
    /**
     * 根据 id 获取用户信息 先从缓存中获取，若没有再调用用户回调获取
     */
    public synchronized void getCachedUser(final String id, final AVCallback<LCChatKitUser> callback) {
        getCachedUsers(Collections.singletonList(id), new AVCallback<List<LCChatKitUser>>() {
            @Override
    ...
    /**
     * 获取多个用户的信息 先从缓存中获取，若没有再调用用户回调获取
     */
    ...
    private void getProfilesFromProvider(List<String> idList, final List<LCChatKitUser> profileList,
        ...
        if (null != profileProvider) {
            profileProvider.fetchProfiles(idList, (userList, e) -> {
                if (null != userList) {
    ...
    /**
     * 缓存 LCChatKitUser 信息，更新缓存同时也更新 db 如果开发者 LCChatKitUser 信息变化，可以通过调用此方法刷新缓存
     */
