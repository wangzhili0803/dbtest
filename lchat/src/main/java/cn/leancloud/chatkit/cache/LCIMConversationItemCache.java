...
/**
 * Created by wli on 16/2/26. 缓存未读消息数量
 * <p/>
 * 流程 1、初始化时从 db 里同步数据到缓存 2、插入数据时先更新缓存，在更新 db 3、获取的话只从缓存里读取数据
 */
public class LCIMConversationItemCache {

    private static final String CONVERSATION_ITEM_TABLE_NAME = "ConversationItem";

    private final Map<String, LCIMConversationItem> conversationItemMap;
    ...
    /**
     * 因为只有在第一次的时候需要设置 Context 以及 clientId，所以单独拎出一个函数主动调用初始化 避免 getInstance 传入过多参数 因为需要同步数据，所以此处需要有回调
     */
    ...
    public synchronized void deleteConversation(String convid) {
        if (!TextUtils.isEmpty(convid)) {
            ...
            conversationItemDBHelper.deleteData(Collections.singletonList(convid));
        }
    }
    ...
    public synchronized List<String> getSortedConversationList() {
        ...
        SortedSet<LCIMConversationItem> sortedSet = new TreeSet<>(conversationItemMap.values());
