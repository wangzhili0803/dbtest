...
import cn.leancloud.chatkit.utils.MD5;
import cn.leancloud.utils.StringUtil;

/**
 * Created by wli on 16/2/25. key value 形式的存储，只能存储 String，其他数据也要转化成 String 存储 因为忽略了具体数据格式，所以更新具体属性的时候必须更新整条记录
 * <p/>
...
class LCIMLocalStorage extends SQLiteOpenHelper {
    ...
    public LCIMLocalStorage(Context context, String clientId, String tableName) {
        ...
        final String md5ClientId = MD5.md5(clientId);
    ...
    public void getIds(final AVCallback<List<String>> callback) {
        if (null != callback) {
            readDbHandler.post(() -> callback.internalDone(getIdsSync(), null));
        }
    }

    /**
     * 根据 key 值获对应的 values 注意：并不保证回调 data 与 id 顺序一致
     *
    ...
    public void getData(final List<String> ids, final AVCallback<List<String>> callback) {
        if (null != callback) {
            if (null != ids && ids.size() > 0) {
                readDbHandler.post(() -> callback.internalDone(getDataSync(ids), null));
            } else {
    ...
    public void insertData(final List<String> idList, final List<String> valueList) {
        if (null != idList && null != valueList && idList.size() == valueList.size()) {
            readDbHandler.post(() -> insertSync(idList, valueList));
        }
    }
    ...
    public void insertData(String id, String value) {
        if (!TextUtils.isEmpty(id) && !TextUtils.isEmpty(value)) {
            insertData(Collections.singletonList(id), Collections.singletonList(value));
        }
    }
    ...
    public void deleteData(final List<String> ids) {
        if (null != ids && !ids.isEmpty()) {
            readDbHandler.post(() -> deleteSync(ids));
        }
    }
    ...
    /**
     * 获取数据，此为同步方法 注意：并不保证回调 data 与 id 顺序一致
     */
