...
/**
 * Created by wli on 15/12/1. 和 Conversation 相关的事件的 handler 需要应用主动调用  AVIMMessageManager.setConversationEventHandler 关于回调会何时执行可以参见
 * https://leancloud.cn/docs/realtime_guide-android.html#添加其他成员
 */
public class LCIMConversationHandler extends AVIMConversationEventHandler {
    ...
    public void onUnreadMessagesCountUpdated(AVIMClient client, AVIMConversation conversation) {
        cn.leancloud.chatkit.cache.LCIMConversationItemCache.getInstance().insertConversation(conversation.getConversationId());
        AVIMMessage lastMessage = conversation.getLastMessage();
        System.out.println(
            "LCIMConversationHandler#onUnreadMessagesCountUpdated conv=" + conversation.getConversationId() + ", lastMsg: " + lastMessage
                .getContent());
    ...
    public void onMessageUpdated(AVIMClient client, AVIMConversation conversation, AVIMMessage message) {
        cn.leancloud.chatkit.utils.LCIMLogUtils.d("message " + message.getMessageId() + " updated!");
        EventBus.getDefault().post(new LCIMMessageUpdatedEvent(message));
    }
